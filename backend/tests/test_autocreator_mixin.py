...
class TestAutoCreatorMixin:
    """Tests for the AutoCreatorMixin that sets creator/author from request.user."""

    def _make_mixin(self, auto_set_fields: dict[str, str]):
    ...
    def test_sets_creator_from_authenticated_user(self):
        user = MagicMock()
        ...
        TestSerializer = self._make_mixin({'creator': 'creator'})
        serializer = TestSerializer(context={'request': mock_request})

        validated_data: dict = {'title': 'test'}
        # The mixin modifies validated_data in-place BEFORE calling super().create()
        # We catch the AttributeError from ModelSerializer trying to access None.model
        try:
    ...
    def test_sets_author_from_authenticated_user(self):
        user = MagicMock()
        type(user).is_anonymous = PropertyMock(return_value=False)
        ...
        TestSerializer = self._make_mixin({'author': 'author'})
        serializer = TestSerializer(context={'request': mock_request})
    ...
    def test_is_noop_for_anonymous_user(self):
        mock_request = MagicMock()
