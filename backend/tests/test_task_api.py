...
class TestTaskListCreate:
    def test_unauthenticated_returns_401(self, db, client: APIClient):
        response = client.get('/api/v1/tasks/')
        assert response.status_code in (status.HTTP_401_UNAUTHORIZED, status.HTTP_403_FORBIDDEN)

    def test_list_returns_paginated_results(self, auth_client: APIClient):
    ...
    def test_list_with_filtering_by_status(self, auth_client: APIClient):
    ...
    def test_list_with_filtering_by_priority(self, auth_client: APIClient):
        creator = User.objects.get(username='testuser')
    ...
    def test_list_with_filtering_by_creator(self, auth_client: APIClient, another_user):
        creator = User.objects.get(username='testuser')
    ...
    def test_list_with_filtering_by_assignee(self, auth_client: APIClient, task_with_assignee):
        assignee = User.objects.get(username='anotheruser')
        response = auth_client.get('/api/v1/tasks/', {'assignee_id': str(assignee.pk)})
        assert response.status_code == status.HTTP_200_OK
        assert response.json()['count'] == 1

    def test_list_with_ordering(self, auth_client: APIClient):
        creator = User.objects.get(username='testuser')
    ...
    def test_list_with_descending_ordering(self, auth_client: APIClient):
        creator = User.objects.get(username='testuser')
        Task.objects.create(title='First', creator=creator)
        Task.objects.create(title='Second', creator=creator)
        response = auth_client.get('/api/v1/tasks/', {'ordering': '-created_at'})
    ...
    def test_create_task(self, auth_client: APIClient):
        creator = User.objects.get(username='testuser')
    ...
    def test_create_task_with_assignee(self, auth_client: APIClient, another_user):
        response = auth_client.post(
            '/api/v1/tasks/',
            {
                'title': 'Assigned Task',
    ...
    def test_create_task_with_invalid_assignee(self, auth_client: APIClient):
...
class TestTaskDetail:
    def test_retrieve_task(self, auth_client: APIClient, task):
    ...
    def test_retrieve_task_with_assignee(self, auth_client: APIClient, task_with_assignee):
    ...
    def test_retrieve_task_null_assignee(self, auth_client: APIClient, task):
        response = auth_client.get(f'/api/v1/tasks/{task.pk}/')
        data = response.json()
        assert data['assignee'] is None

    def test_retrieve_nonexistent_task(self, auth_client: APIClient):
        from uuid import uuid4
        response = auth_client.get(f'/api/v1/tasks/{uuid4()}/')
        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_patch_task(self, auth_client: APIClient, task):
    ...
    def test_patch_creator_is_readonly(self, auth_client: APIClient, task, another_user):
        response = auth_client.patch(
            f'/api/v1/tasks/{task.pk}/',
    ...
    def test_delete_task(self, auth_client: APIClient, task):
        response = auth_client.delete(f'/api/v1/tasks/{task.pk}/')
        assert response.status_code == status.HTTP_204_NO_CONTENT
        assert not Task.objects.filter(pk=task.pk).exists()

    def test_delete_nonexistent_task(self, auth_client: APIClient):
        from uuid import uuid4
        response = auth_client.delete(f'/api/v1/tasks/{uuid4()}/')
        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_any_user_can_manage_any_task(self, auth_client: APIClient, task, another_user):
        response = auth_client.delete(f'/api/v1/tasks/{task.pk}/')
        assert response.status_code == status.HTTP_204_NO_CONTENT
