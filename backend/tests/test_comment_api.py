...
def test_unauthenticated_returns_401(db, client: APIClient, task):
    response = client.get(f'/api/v1/tasks/{task.pk}/comments/')
    assert response.status_code in (status.HTTP_401_UNAUTHORIZED, status.HTTP_403_FORBIDDEN)


def test_list_returns_empty_for_task_with_no_comments(auth_client: APIClient, task):
...
def test_list_returns_comments_for_task(auth_client: APIClient, task, another_user):
...
def test_list_comments_only_for_specified_task(auth_client: APIClient, task, task_with_assignee, another_user):
...
def test_create_comment(auth_client: APIClient, task):
...
def test_create_comment_with_whitespace_body_returns_400(auth_client: APIClient, task):
    response = auth_client.post(
        f'/api/v1/tasks/{task.pk}/comments/',
        {'body': '   '},
        format='json',
...
def test_create_comment_with_empty_body_returns_400(auth_client: APIClient, task):
    response = auth_client.post(
        f'/api/v1/tasks/{task.pk}/comments/',
        {'body': ''},
        format='json',
...
def test_create_comment_with_body_exceeding_max_length_returns_400(auth_client: APIClient, task):
    body = 'x' * 10001
    response = auth_client.post(
        f'/api/v1/tasks/{task.pk}/comments/',
        {'body': body},
...
def test_create_comment_with_body_at_max_length_succeeds(auth_client: APIClient, task):
    body = 'x' * 10000
    response = auth_client.post(
        f'/api/v1/tasks/{task.pk}/comments/',
        {'body': body},
...
def test_create_comment_author_is_set_from_request_user(auth_client: APIClient, task, another_user):
    response = auth_client.post(
        f'/api/v1/tasks/{task.pk}/comments/',
...
def test_retrieve_comment(auth_client: APIClient, task):
...
def test_retrieve_comment_for_nonexistent_task_returns_404(auth_client: APIClient):
    response = auth_client.get(f'/api/v1/tasks/{uuid4()}/comments/{uuid4()}/')
    assert response.status_code == status.HTTP_404_NOT_FOUND


def test_patch_comment(auth_client: APIClient, task):
...
def test_patch_any_user_can_update_any_comment(auth_client: APIClient, task, another_user):
...
def test_delete_comment(auth_client: APIClient, task):
...
def test_delete_nonexistent_comment(auth_client: APIClient, task):
    response = auth_client.delete(f'/api/v1/tasks/{task.pk}/comments/{uuid4()}/')
    assert response.status_code == status.HTTP_404_NOT_FOUND


def test_delete_any_user_can_delete_any_comment(auth_client: APIClient, task, another_user):
...
def test_deleting_task_deletes_comments(db, task):
...
def test_deleting_user_sets_author_to_null(db, task, another_user):
