...
def test_retrieve_returns_full_representation(auth_client: APIClient, db, user):
...
def test_retrieve_includes_creator(auth_client: APIClient, db, user):
...
def test_retrieve_assignee_is_null_when_unassigned(auth_client: APIClient, db, user):
...
def test_retrieve_assignee_includes_user_info_when_assigned(
    auth_client: APIClient, db, user, another_user
):
    task = Task.objects.create(
...
def test_retrieve_includes_timestamps(auth_client: APIClient, db, user):
    task = Task.objects.create(title='Task', creator=user)
    response = auth_client.get(f'/api/v1/tasks/{task.pk}/')
    assert response.status_code == 200
    data = response.json()
